...
import { insertFile, getPoolStatus } from "./db";
...
app.post("/file-changed", async (req: Request<unknown, unknown, FileChangedRequestBody>, res: Response): Promise<void> => {
  const startTime = Date.now();
  const { file, event } = req.body;

  // Input validation
  if (!file || !event) {
    console.warn(`⚠️  Invalid request: missing required fields. File: "${file}", Event: "${event}"`);
    res.status(400).json({
      error: "Bad Request",
  ...
  console.log(`📥 File change notification: ${file} (${event})`);
